...
const ARROW_SIZE = 48;
const ZF = "zf-";
const HOVER_HIGHLIGHT = "feature-hover-highlight";
...
function syncPerFeatureLayersSorted(
  ...
  positionHoverHighlight(map);
}

function positionHoverHighlight(map: maplibregl.Map) {
  if (!map.getLayer(HOVER_HIGHLIGHT)) return;
  const layers = map.getStyle().layers ?? [];
  const firstSymbol = layers.find(
    (l) => l.id.startsWith(ZF) && l.type === "symbol",
  );
  if (firstSymbol) {
    map.moveLayer(HOVER_HIGHLIGHT, firstSymbol.id);
  } else if (map.getLayer(FIRST_OVERLAY)) {
    map.moveLayer(HOVER_HIGHLIGHT, FIRST_OVERLAY);
  }
}
...
function polygonArea(geom: GeoJSON.Geometry): number {
  if (geom.type === "Polygon") {
    return ringArea(geom.coordinates[0] as number[][]);
  }
  if (geom.type === "MultiPolygon") {
    return geom.coordinates.reduce(
      (acc, poly) => acc + ringArea(poly[0] as number[][]),
      0,
    );
  }
  return 0;
  ...
  const geojsonFeatures: GeoJSON.Feature[] = sorted
    .flatMap((f): GeoJSON.Feature[] => {
      ...
      const props = {
        ...
        color: f.color,
        opacity: f.opacity,
        hoverColor: f.hoverColor || f.color,
        area: f.type === "polygon" ? polygonArea(displayGeometry) : 0,
        order: f.order,
...
export { FEATURES_SOURCE, ZF, HOVER_HIGHLIGHT, CHOROPLETH_SOURCE, CHOROPLETH_OUTLINE_SOURCE, CHOROPLETH_FILL, CHOROPLETH_BORDER, CHOROPLETH_OUTLINE, CHOROPLETH_HIT, CHOROPLETH_BORDER_OPACITY, CHOROPLETH_OUTLINE_OPACITY };
